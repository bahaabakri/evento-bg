...
export class AuthService {
  ...
  async registerLoginUser(
    ...
    return this._buildResponse(
      user,
      `${message}, and Otp has been sent to your email address`,
    );
  }
   ...
   * resend otp for user
   */
  async resendUserOtp(email: string): Promise<{ user: User; message: string }> {
  ...
  async resendAdminOtp(
    ...
    let admin = await this._userService.findAdminByEmail(email);
    if (!admin) {
  ...
  async registerAdmin(
    ...
    return this._buildResponse(
      admin,
      'Admin Created Successfully, and Otp has been sent to your email address',
    );
  }
  ...
  async loginAdmin(
    ...
    return this._buildResponse(
      admin,
      'Admin Logged in Successfully, and Otp has been sent to your email address',
    );
  }
  /**
   * Verify user
  ...
  async verifyUser(
    email: string,
    enteredOtp: string,
  ): Promise<{ user: User; message: string; access_token: string }> {
    // get user
